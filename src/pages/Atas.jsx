...
export default function Atas() {
  ...
  const abrir = (ata = null) => {
    if (ata) {
      setForm({ tipo:ata.tipo, outro_tipo:ata.outro_tipo||'', data:ata.data, hora:ata.hora||'', abertura:ata.abertura||'', louvor:ata.louvor||'', palavra_inicial:ata.palavra_inicial||'', registro:ata.registro||'', presentes:ata.presentes||[], votacoes:ata.votacoes||[] })
      setEditId(ata.id)
    } else {
  ...
  const salvar = async () => {
    ...
    setLoading(true)
    const row = { tipo:form.tipo, outro_tipo:form.outro_tipo||null, data:form.data, hora:form.hora||null, abertura:form.abertura||null, louvor:form.louvor||null, palavra_inicial:form.palavra_inicial||null, registro:form.registro||null, presentes:JSON.stringify(form.presentes), votacoes:JSON.stringify(form.votacoes) }
    if (editId) {
