...
const dateStyle = (tipo) => {
  const local = tipo === 'Igreja Local'
  return {
    background: local ? 'rgba(249,115,22,.1)' : 'var(--cdim)',
    border: local ? '1px solid rgba(249,115,22,.35)' : '1px solid var(--cgl)',
    borderRadius: 8, padding: '5px 9px', textAlign: 'center', flexShrink: 0, minWidth: 46,
  }
}
const dateNumColor = (tipo) => tipo === 'Igreja Local' ? '#f97316' : 'var(--cy)'
...
export default function Agenda() {
  ...
  const abrir = (ev = null) => {
    if (ev) {
      setForm({ data: ev.data, hora: ev.hora||'', titulo: ev.titulo, descricao: ev.desc||ev.descricao||'', tipo: ev.tipo||'Culto' })
      setEditId(ev.id)
    } else {
      ...
      {lista.length===0 ? <Empty icon="📅" text="Nenhum evento cadastrado." /> : lista.map(ev => {
        ...
        return (
          <div key={ev.id} style={{display:'flex',alignItems:'flex-start',gap:12,padding:'11px 0',borderBottom:'1px solid var(--bd)',opacity:passado?.4:1}}>
            <div style={dateStyle(ev.tipo)}>
              <div style={{fontFamily:'var(--font-display)',fontSize:20,color:c,lineHeight:1}}>{d.getDate()}</div>
              <div style={{fontSize:8,color:c,letterSpacing:2,textTransform:'uppercase'}}>{MESES_A[d.getMonth()]} {d.getFullYear()}</div>
            </div>
            <div style={{flex:1}}>
