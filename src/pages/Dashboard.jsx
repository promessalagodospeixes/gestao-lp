...
import { MESES_A, DISP_OPTS, fmtBR, nextWeekend, getSabDom, getCultosOrdenados, waLink, primeiroUltimo, nomeDisp } from '../lib/utils.js'
          ...
          {fns.map(k => {
            ...
            return (
              <div key={k} style={{ display:'flex', alignItems:'center', padding:'4px 0', borderBottom:'1px solid var(--bd)', gap:8, fontSize:11 }}>
                ...
                <div style={{ color: v === nome ? 'var(--cy)' : 'var(--tx)', fontWeight: v === nome ? 700 : 500 }}>{v ? nomeDisp(v, membros) : <span style={{color:'var(--g)'}}>—</span>}</div>
              </div>
            )
          })}
          {/* Equipe de Louvor */}
              ...
              : <>
                  {vocals.length > 0 && (
                    <div style={{ display:'flex', alignItems:'flex-start', padding:'3px 0', gap:8, fontSize:11 }}>
                      ...
                      <div style={{ color:'var(--tx)', display:'flex', flexWrap:'wrap', gap:4 }}>
                        {vocals.map((v,i) => (
                          <span key={i} style={{ color: v===nome?'var(--cy)':'var(--tx)', fontWeight: v===nome?700:500 }}>{nomeDisp(v, membros)}{i<vocals.length-1?',':''}</span>
                        ))}
                      </div>
                    </div>
                  )}
                  {instArr.map(([papel, v]) => {
                    ...
                    const nomes = Array.isArray(v)
                      ? v.filter(x=>x?.nome).map(x=>nomeDisp(x.nome, membros))
                      : v ? [nomeDisp(v, membros)] : []
