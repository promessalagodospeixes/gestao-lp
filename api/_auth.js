...
export function verificadorRecibo(partes) {
  return crypto.createHmac('sha256', SEGREDO).update(String(partes))
    .digest('base64url').replace(/[^A-Z0-9]/gi, '').toUpperCase().slice(0, 4)
}
